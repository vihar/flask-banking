...
from . models import User, Posts, db, Accounts
from . forms import AddPostForm, SignUpForm, SignInForm, AboutUserForm, AddAccForm
...
def acc_view():
    if session['user_available']:
        ...
        user = User.query.all()
        return render_template('acc_details.html', accs=accs, user=user)
    flash('User is not Authenticated')
    return redirect(url_for('index'))
